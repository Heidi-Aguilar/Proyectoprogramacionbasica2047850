...
void Alta()
{
	...
	for (int i = 0; i < alta; i++)
	{
		...
		for (int i=0;i< alta; i++)
        {
	         ...
	         scanf("%d", &base[i].numitem);
	         for (int j=-1;j<i; j++)
	         {
	             if (base[i].numitem == base[j].numitem)
	             {
	                 do
	                 {
	                     printf("Este numero de item esta ocupado");
	                     scanf("%d",&base[i].numitem);
	                 } while (base[i].numitem == base[j].numitem);
	             }
	         }
	         while (getchar()!='\n');
		    cin.ignore();
